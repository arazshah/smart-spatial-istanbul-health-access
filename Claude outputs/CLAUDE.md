...
**Phase 3 is written but not yet executed (2026-09-19).**
`notebooks/02_rule_based_arm.ipynb` implements Arm 1 end to end against the
real, verified 0.3.0 API — `crs_transform`, a CRS-mismatch smoke test,
`find_nearest_neighbors` (boundary + centroid), `spatial_join_features` for
the per-mahalle facility count, `dissolve_features` called directly (not
planner-reachable), `rank_features` + `build_report`. It also resolves two
of `paper/PLAN.md`'s "Open decisions" by source-reading and design
(`spatial_join` over `filter_points_in_polygon`; the CRS-mismatch safety
net's real behavior) — **both still need a real run to confirm**, per this
file's own rule against trusting an unexecuted notebook. Same
delivery/verification loop as phase 2: it needs to run on a machine with
the pin actually installed, then its real output gets checked here before
`paper/PLAN.md`'s "Findings" section or this file are updated again.
