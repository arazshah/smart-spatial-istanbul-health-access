...
| 3 | Rule-based arm | `notebooks/02_rule_based_arm.ipynb`, `results/rule_based_*.{csv,json}` — **written 2026-09-19, not yet executed** (see notebook's own warning) | no | no |
...
- **✅ Resolved 2026-09-19 (by source reading; not yet empirically
  triggered): the CRS-mismatch safety net on `find_nearest_neighbors`
  (Vienna's candidate Bug 5) is present and correct at 0.3.0, with a
  caveat.** `plugins/distance_calculator.py`'s `_raise_if_crs_mismatch()`
  ...
  `notebooks/02_rule_based_arm.ipynb` §3 has a smoke test that deliberately
  triggers this — **still needs a real run to confirm it behaves as the
  source says**; if it doesn't, file a `bugs/` report.
