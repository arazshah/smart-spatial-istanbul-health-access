...
| 4 | LLM arm (N=20) | `notebooks/03_llm_arm.ipynb`, `results/llm_runs/` — **`bugs/002` and `bugs/003` both fixed upstream (2026-09-19)**; pin bumped 0.3.0 → 0.4.1 → 0.4.2. Waiting on the author to reinstall at the new pin and re-run the smoke test (§1-4) once more before the N=20 batch — not yet empirically confirmed clean at 0.4.2. | **yes** | **yes** |
...
- **Fixed upstream in `smart_spatial_system` 0.4.2** (2026-09-19),
  ...
  `bugs/003`'s "Resolution" section has the full diff. Waiting on the
  author to reinstall and re-run the smoke test once more (not yet
  empirically confirmed clean at 0.4.2) before the N=20 batch.
